    ...
    public:
        bool create(bool bUseDeviceFrame, int gpu_id, cudaVideoCodec eCodec, bool bLowLatency = false,
            ...
            if(cuContext == nullptr){
                spdlog::info("Info: Current Context is nullptr.");
                return false;
            }
        ...
        CUcontext get_context() override  { return cuContext; }

        int get_bit_depth() override { assert(m_nWidth); return m_nBitDepthMinus8 + 8; }

        unsigned int get_frame_index() override { return m_iFrameIndex; }

        CUVIDEOFORMAT get_video_format_info() override { assert(m_nWidth); return m_videoFormat; }
