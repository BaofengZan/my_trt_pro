...
namespace FFHDDemuxer{
                ...
                *pnVideoBytes = m_pktFiltered.size;
                ...
                if (m_bMp4MPEG4 && (m_frameCount == 0)) {
                    ...
                    if (extraDataSize > 0) {
                        ...
                        if (!m_pDataWithHeader) {
                            spdlog::info("FFmpeg error, m_pDataWithHeader alloc failed");
                            return false;
                        }
    ...
    private:
        ...
        bool open(AVFormatContext *fmtc, int64_t timeScale = 1000 /*Hz*/) {
            if (!fmtc) {
                spdlog::info("No AVFormatContext provided.");
                return false;
            }
            ...
            if (m_iVideoStream < 0) {
                spdlog::info("FFmpeg error: Could not find stream in input file");
                return false;
            }
            ...
            if (m_bMp4H264) {
                ...
                if (!bsf) {
                    spdlog::info("FFmpeg error: av_bsf_get_by_name() failed");
                    return false;
                }
